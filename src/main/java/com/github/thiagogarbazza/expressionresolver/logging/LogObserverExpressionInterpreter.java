...
public class LogObserverExpressionInterpreter {
  ...
  @Around(POINTCUT)
  public Object logObserverExpressionInterpreter(ProceedingJoinPoint joinPoint) throws Throwable {
    LocalDateTime start = LocalDateTime.now();
    if (log.isInfoEnabled()) {
      log.info(format(MSG_LOG_START, toStringSignatureBuilder(joinPoint)));
    }
    ...
    if (log.isInfoEnabled()) {
      log.info(format(MSG_LOG_ENDING, toStringSignatureBuilder(joinPoint), response, Duration.between(start, LocalDateTime.now())));
    }
